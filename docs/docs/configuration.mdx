...
The verdict is cached for 15 minutes — the check transcribes a real audio sample, so it is not
re-run per request — and `POST /bots` refuses on it for exactly that long. After fixing the value,
call `GET /health?force=1` to re-probe immediately rather than waiting the cache out.
